...
class SwipeActionButton: UIButton {
    ...
    var alignmentRect: CGRect {
            let titleHeight = titleBoundingRect(with: CGSize(width: bounds.width, height: CGFloat.greatestFiniteMagnitude)).integral.height
    ...
    override var intrinsicContentSize: CGSize {
            if #available(iOS 15.0, *) {
                return CGSize(width: UIView.noIntrinsicMetric, height: alignmentRect.height)
            } else {
    ...
    func preferredWidth(maximum: CGFloat) -> CGFloat {
        ...
        if #available(iOS 15.0, *) {
                   return min(width, max(textWidth, imageWidth))
               } else {
